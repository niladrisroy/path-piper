...
const CHILD_SAFE_VOCABULARY = [
  'happy', 'fun', 'play', 'learn', 'study', 'friend', 'school', 'teacher', 'book',
  'game', 'art', 'music', 'dance', 'sport', 'science', 'math', 'reading', 'writing',
  'adventure', 'explore', 'create', 'build', 'design', 'project', 'team', 'help',
  'share', 'kind', 'nice', 'good', 'great', 'awesome', 'cool', 'amazing', 'wonderful',
  'beautiful', 'smart', 'creative', 'talented', 'brave', 'strong', 'gentle', 'caring',
  'family', 'parent', 'sibling', 'pet', 'nature', 'animal', 'flower', 'tree',
  'sunshine', 'rainbow', 'star', 'moon', 'earth', 'water', 'food', 'healthy',
  'exercise', 'sleep', 'dream', 'hope', 'wish', 'birthday', 'celebration', 'gift'
]

// Safe phrase suggestions for common scenarios
const SAFE_PHRASE_SUGGESTIONS = {
  'frustration': ['I need help with this', 'This is challenging for me', 'I\'m working hard on this'],
  'disagreement': ['I have a different idea', 'Let\'s talk about this', 'I respect your opinion'],
  'sadness': ['I\'m feeling down today', 'I need some encouragement', 'This is hard for me'],
  'excitement': ['I\'m really happy about this!', 'This is so cool!', 'I can\'t wait to share this!'],
  'achievement': ['I worked really hard on this', 'I\'m proud of my progress', 'I learned something new today']
}
...
async function performComprehensiveModeration(
  ...
  violencePatterns.forEach(pattern => {
    ...
    if (matches) {
      ...
      suggestions.push('Content contains violent language not suitable for children')
    }
  })
  ...
  selfHarmPatterns.forEach(pattern => {
    ...
    if (matches) {
      ...
      requiresHumanReview = true
      suggestions.push('If you\'re having difficult feelings, please talk to a trusted adult or counselor')
    }
  })
  ...
  personalInfoPatterns.forEach(pattern => {
    ...
    if (matches) {
      ...
      requiresHumanReview = true
      suggestions.push('Never share personal information like addresses, phone numbers, or email addresses online')
    }
  })
  ...
  strangerDangerPatterns.forEach(pattern => {
    ...
    if (matches) {
      ...
      requiresHumanReview = true
      suggestions.push('Never arrange to meet strangers online or share personal information')
    }
  })
  ...
  substancePatterns.forEach(pattern => {
    ...
    if (matches) {
      ...
      suggestions.push('Content about drugs and alcohol is not appropriate for children')
    }
  })

  // 8. PRE-APPROVED VOCABULARY CHECK
  const words = content.toLowerCase().split(/\s+/)
  const nonApprovedWords = words.filter(word => 
    word.length > 2 && 
  ...
  if (riskScore >= 25 || flags.some(flag => ['self_harm', 'violence_threats', 'stranger_danger'].includes(flag))) {
    status = 'rejected'
    reason = 'Content contains highly inappropriate or dangerous material'
    requiresHumanReview = true
  } else if (riskScore >= 15 || flags.some(flag => ['bullying', 'personal_information'].includes(flag))) {
    status = 'pending_review'
    reason = 'Content requires human review before publication'
    requiresHumanReview = true
  } else if (riskScore >= 5 || flags.some(flag => ['explicit_language', 'substance_references'].includes(flag))) {
    status = 'flagged'
  ...
  try {
    ...
    await prisma.humanReviewQueue.create({
      data: {
        ...
        flags: result.flags,
        reason: result.reason,
        priority: result.riskScore >= 20 ? 'high' : result.riskScore >= 10 ? 'medium' : 'low',
        queuedAt: new Date()
      }
    })
  } catch (error) {
