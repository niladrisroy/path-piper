  ...
  try {
    ...
    if (apiStatus.googleVision.enabled) {
      try {
        ...
        apiStatus.googleVision.status = response.status === 400 ? 'online' : 'error'
      } catch {
    ...
    return NextResponse.json({
      ...
      apis: apiStatus,
      summary: {
        totalEnabled: Object.values(apiStatus).filter(api => api.enabled).length,
        totalOnline: Object.values(apiStatus).filter(api => api.status === 'online').length
      }
    })
  } catch (error) {
