...
export async function GET(request: NextRequest) {
  try {
    ...
    if (reviewedItems.length > 0) {
      ...
      avgResponseTime = Math.round(totalResponseTime / reviewedItems.length / (1000 * 60)) // Convert to minutes
    }
    ...
    const topFlags = flagCounts
      .flatMap(item => item.flags.map(flag => ({ flag, count: item._count.flags })))
      .reduce((acc, { flag, count }) => {
