...
import { AlertTriangle, CheckCircle, Clock, XCircle, Eye, Flag, Shield, Users, Activity, TrendingUp, Image, Video, AlertCircle } from "lucide-react"
...
export default function ModerationDashboard() {
  ...
  const [filter, setFilter] = useState<'pending' | 'reviewed' | 'high_risk'>('pending')
  ...
  const fetchModerationItems = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/admin/moderation?filter=${filter}&limit=50`)
  ...
  return (
    <div className="container mx-auto p-6 max-w-7xl">
      <div className="mb-6">
        ...
        <p className="text-gray-600">Child-Safe Content Review & Moderation System</p>
      </div>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <CardTitle className="text-lg flex items-center gap-2">
                            {item.contentType.charAt(0).toUpperCase() + item.contentType.slice(1)} by {item.user.firstName} {item.user.lastName}
                        ...
                        <div className="border-t pt-4">
                          <div className="space-y-3">
                            <div>
                              ...
                              <Textarea
                                value={selectedItem?.id === item.id ? reviewReason : ''}
                                onChange={(e) => {
                            ...
                            <div>
                              <label className="text-sm font-medium">Suggestions for User (optional):</label>
                              <Textarea
                                value={selectedItem?.id === item.id ? reviewSuggestions : ''}
                                onChange={(e) => {
                                  setSelectedItem(item)
                            ...
                            <div className="flex gap-2">
                              <Button
                                ...
                                disabled={selectedItem?.id === item.id && !reviewReason.trim()}
                              >
                      ...
                      {item.reviewer && (
                        <div className="border-t pt-4">
                          <div className="text-sm text-gray-600">
                            <p><strong>Reviewed by:</strong> {item.reviewer.firstName} {item.reviewer.lastName}</p>
                            <p><strong>Review time:</strong> {new Date(item.reviewedAt!).toLocaleString()}</p>
                            {item.reviewerReason && (
                              <p><strong>Reason:</strong> {item.reviewerReason}</p>
                            )}
                            {item.reviewerSuggestions && item.reviewerSuggestions.length > 0 && (
                              <div>
                                <strong>Suggestions:</strong>
                                <ul className="list-disc list-inside mt-1">
                                  {item.reviewerSuggestions.map((suggestion, idx) => (
