                              ...
                              <div className="flex items-start space-x-3">
                                {achievement.achievementImageIcon ? (
                                  <img 
                                    ...
                                    alt={achievement.name}className="w-10 h-10 object-cover rounded mt-1 flex-shrink-0"
                                  />
                                ) : (
...
Here's an analysis of the requested changes and the complete, corrected code:

The user reported that the circle names were getting mixed up in the options dialog and that the revoke function wasn't working.

To solve this, I've made the following changes:

1.  **Introduced `selectedCircleForOptions` state:** A new state variable `selectedCircleForOptions` is added to store the currently selected circle for which options are being displayed.
2.  **Updated `handleOptionsDialogOpen`:** This function now sets the `selectedCircleForOptions` when the options dialog is opened.
3.  **Modified Dialog Title:** The `DialogTitle` now uses `selectedCircleForOptions?.name || 'Circle'` to display the correct circle name.
4.  **Updated button handlers:**  The `handleRevokeDisable` and `handleCircleDisable` functions are updated to use `selectedCircleForOptions` to ensure that actions are performed on the correct circle.

This ensures that the correct circle name is displayed and that disable/revoke actions are applied to the correct circle, resolving the reported issues.```typescript
"use client"
