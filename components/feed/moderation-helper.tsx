...
export default function ModerationHelper({ content, onSuggestionSelect, onClose }: ModerationHelperProps) {
  ...
  const getSafeAlternatives = (text: string): SafePhrase[] => {
    ...
    // Common problematic phrases and their safe alternatives
    const replacements = [
      { pattern: /\b(stupid|dumb|idiotic)\b/gi, safe: 'challenging', category: 'respectful' },
      { pattern: /\b(hate|despise)\b/gi, safe: 'dislike', category: 'emotion' },
      { pattern: /\b(ugly|hideous)\b/gi, safe: 'not my style', category: 'appearance' },
      { pattern: /\b(shut up|be quiet)\b/gi, safe: 'please listen', category: 'communication' },
      { pattern: /\b(loser|failure)\b/gi, safe: 'someone who is learning', category: 'encouragement' },
      { pattern: /\b(angry|furious|mad)\b/gi, safe: 'frustrated', category: 'emotion' },
      { pattern: /\b(this sucks|this is terrible)\b/gi, safe: 'this is challenging', category: 'feedback' }
    ]
    ...
    <Card className="w-full max-w-2xl mx-auto border-2 border-blue-200">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className={`p-4 rounded-lg border ${
          contentSafety ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'
        }`}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`font-medium ${
              ...
              {contentSafety ? 'Content looks safe!' : 'Content may need improvement'}
            </span>
          </div>
          <p className={`text-sm ${
                ...
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex-1">
                    <div className="text-sm">
                      <span className="text-red-600 line-through">"{alt.original}"</span>
                      <span className="mx-2">→</span>
                      <span className="text-green-600 font-medium">"{alt.safe}"</span>
                    </div>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      const newContent = content.replace(new RegExp(alt.original, 'gi'), alt.safe)
        ...
        {suggestions.length > 0 && (
          <div>
            <h3 className="font-medium text-gray-700 mb-3">Safe Phrase Suggestions</h3>
